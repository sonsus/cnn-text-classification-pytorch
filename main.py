...
parser.add_argument('--scatterlab', type=bool, default=False, help='apply project data from scatterlab')
parser.add_argument('--debug', action='store_true', default=False, help="debugmode (shorter dataset loaded)")
parser.add_argument('--scheduler', type=str, default='linear', help="scheduler: cosine, linear")
...
if args.scatterlab:
    wandb.init(project="cnn-sent_cls (Yoon et al 2014)")
    if args.debug:
        args.batch_size = 6
        args.log_interval=1
    EXPCONF.debug = args.debug
    EXPCONF.lr = args.lr
...
args.embed_num = len(vocab.itos)
args.class_num = len(label_field.vocab) - 1 if not args.scatterlab else 2  # for scatterlab prob is binary
